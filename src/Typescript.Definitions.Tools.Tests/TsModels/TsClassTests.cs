...
namespace Typescript.Definitions.Tools.Tests.TsModels
{
    public class TsClassTests
    {
        ...
        {
            var target = new TsClass(typeof(Outer.Inner));

            Assert.NotNull(target.Module);
            Assert.Equal(typeof(Outer.Inner).Namespace + ".Outer", target.Module.Name);
        }
        ...
        public void WhenModuleIsSet_ClassIsAddedToModule()
        {
            ...
            var target = new TsClass(typeof(Address)) {Module = module};
        ...
        public void WhenModuleIsSetToOtherModule_ClassIsRemovedFromOriginalModule()
        {
            ...
            var module = new TsModule("Tests");
            var target = new TsClass(typeof(Address)) {Module = originalModule};
